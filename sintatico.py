...
def synchronize(recovery_point):
    while (
        current_token_value() not in recovery_point
        and current_token_value() != ""
    ):
        next_token()
...
def save_error( e: SyntaxError, recovery_point=None):
    ...
    message = f"{e.msg}, received '{current_token_value()}' in line {current_token_line()}"
...
def SyntacticParse(tokens_list: list):
